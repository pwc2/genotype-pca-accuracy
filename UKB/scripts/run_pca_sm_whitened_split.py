...
hl.init(tmp_dir='gs://ukb-data/tmp/ukb-split')
...
print(f'Running PCA/SM on odd/even chromosome split UKB, {samples}...')
...
for chromosomes in ['odd', 'even']:
    print(f'{chromosomes} chromosomes:')
    block_table_fname = f'{chromosomes}-block_table-ws{whiten_ws}'
    block_table_ht = f'{gcs_prefix}/{config_folder}/{block_table_fname}.ht'
    scores_fname = f'{chromosomes}-scores-ws{whiten_ws}-k{k_pcs}'
    scores_ht = f'{gcs_prefix}/{config_folder}/{scores_fname}.ht'
    loadings_fname = f'{chromosomes}-loadings-ws{whiten_ws}-k{k_pcs}'
    loadings_ht = f'{gcs_prefix}/{config_folder}/{loadings_fname}.ht'

    # Load full MatrixTable, filter to odd/even chromosomes, create whitened TSM 
    mt = hl.read_matrix_table(f'{gcs_prefix}/{mt_name}')
    if chromosomes == 'even':
        mt = mt.filter_rows(hl.int(mt.locus.contig) % 2 == 0)
    else:
        mt = mt.filter_rows(hl.int(mt.locus.contig) % 2 != 0)
    mt = mt.checkpoint(f'{gcs_prefix}/checkpoints/{chromosomes}-{mt_name}', overwrite=False, _read_if_exists=True)
    m_variants, n_samples = mt.count()
    tsm = _make_tsm_from_call(mt.GT, **make_tsm_from_call_args)
    tsm.block_table.checkpoint(block_table_ht, overwrite=False, _read_if_exists=True)
